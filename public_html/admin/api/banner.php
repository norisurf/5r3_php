...
switch ($method) {
    ...
    case 'POST':
        ...
        try {
            ...
            $stmt = $db->prepare("INSERT INTO banners (id, mode, image_url, link_url, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, 1, ?, ?)");
            $stmt->execute([$id, $mode, $imageUrl, $linkUrl, $now, $now]);
            ...
            error_log('banner.php POST: ' . $e->getMessage());
            jsonResponse(['error' => 'バナーの保存に失敗しました'], 500);
        }
        break;
