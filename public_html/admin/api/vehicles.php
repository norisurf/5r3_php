...
switch ($method) {
    ...
    case 'POST':
        ...
        try {
            ...
            $stmt = $db->prepare('INSERT INTO vehicles (id, manage_number, title, price, images, basic_info, detailed_info, equipment, description, display_on_lp, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, NOW(), NOW())');
            $stmt->execute([
                $id,
                $manageNumber,
                $body['title'] ?? 'No Title',
