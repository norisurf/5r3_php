...
function fetchFromUrl() {
    ...
    if (btn) btn.disabled = true;
    if (btnText) btnText.textContent = '車両情報を取得中...';
    ...
    fetch('/admin/api/fetch_and_parse.php', {
        ...
        .then(function (data) {
            ...
            if (btnText) btnText.textContent = '画像をダウンロード中...';
            var imageUrls = data.images || [];
            if (imageUrls.length === 0) {
                applyParsedData(data);
                alert('URLからの取得が完了しました（画像なし）');
                return;
            }
            return fetch('/admin/api/download_images.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-Token': getCsrfToken()
                ...
                .then(function (dlResult) {
                    if (dlResult.localPaths) { data.images = dlResult.localPaths; }
                    applyParsedData(data);
                    var saved = dlResult.saved || 0;
                    var total = (dlResult.localPaths || []).length;
                    alert('URLからの取得が完了しました\n画像 ' + saved + '/' + total + ' 枚をローカル保存しました');
                });
        })
        .catch(function (err) {
            if (err !== 'parse error') {
